...
class CourseTabView(EdxFragmentView):
    ...
    def url_to_enroll(course_key):
        ...
        url_to_enroll = reverse(course_about, args=[unicode(course_key)])
        ...
        if request.user.is_anonymous():
            register_warning_message(
                ...
                Text(_("To see course content, {sign_in_link} or {register_link}.")).format(
                    sign_in_link=HTML(
                        '<a href="/login?next=' + urlquote_plus(request.path) + '">{sign_in_label}</a>').format(
                        sign_in_label=_("Sign in"),
                    ),
                    register_link=HTML(
                        '<a href="/register?next=' + urlquote_plus(request.path) + '">{register_label}</a>').format(
            ...
            register_warning_message(
                ...
                Text(_('You must be enrolled in the course to see course content. {enroll_button}')).format(
                    enroll_button=HTML(
                        '<a href="{url_to_enroll}">{enroll_button_label}</a>'.format(
                            url_to_enroll=CourseTabView.url_to_enroll(course_key),
